		...
		try {
			...
			if(newAuth.equalsIgnoreCase("yes"))
			{
				System.out.println("Enter Author Details - Name, city, Qualification ");
				 auth=new Author(sc.next(),sc.next(),sc.next());
				System.out.println(ad.insertRecord(auth));
			}
			else {
...
//			Book b=new Book(i);// this is to delete transient object   
 ...
 public String deleteBookWithJpql(String t){
	 ...
	 Transaction tx=s.beginTransaction();
		try {
			Book b=(Book) s.createQuery("select b from Book b where b.bookTitle=:t").setParameter("t",t).getSingleResult();
			if(b!=null)
				s.delete(b);
			tx.commit();
			msg="Book Deleted "+t;
...
public List<Book> getAllBookForAuthor(long id){
	 ...
	 AuthorDaoImpl ad=new AuthorDaoImpl();
		try {
			...
			bookList=s.createQuery("select b from Book b where b.author=:auth", Book.class).setParameter("auth", a).getResultList();
			tx.commit();			
		}
		catch (RuntimeException e) {
			if (tx != null)
